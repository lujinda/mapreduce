...
// worker.
func (wk *Worker) DoTask(arg *DoTaskArgs, reply *TaskReply) error {
	...
	case mapPhase:
		...
		files := doMap(arg.JobName, arg.TaskNumber, arg.File, arg.NumOtherPhase, wk.Map)
		reply.Files = files
	case reducePhase:
		for file, address := range arg.MapFileAddressByFile {
			if _, err := os.Stat(file); err != nil { //  os.ErrNotExist
				wk.pullFile("Worker", address, file)
			}
		}
		file := doReduce(arg.JobName, arg.TaskNumber, arg.NumOtherPhase, wk.Reduce)
...
func RunWorker(MasterAddress string, me string,
	...
	wk.nRPC = nRPC
	wk.MasterAddress = MasterAddress // 登记一下Master的位置. 数据分片都在master中
	rpcs := rpc.NewServer()
	rpcs.Register(wk)
